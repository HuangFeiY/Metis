...
class IntentMarryUp(nn.Module):
    def __init__(self, pretrained_embed, config=None, label_size=None):
        ...
        if self.config.marryup_type in 'input':
            ...
            self.tag_embedding = nn.Parameter(torch.randn((self.label_size, self.RE_D)),
                                              requires_grad=True)  # B x C, C x D -> B x D
    ...
    def forward(self, input, lengths, re_tags):
        ...
        if self.config.rnn in ['RNN', 'LSTM', 'GRU']:
            pack_padded_seq_input = torch.nn.utils.rnn.pack_padded_sequence(input, lengths.cpu(), batch_first=True,
