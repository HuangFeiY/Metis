...
class BRNNIntegrateEmptyStateSaperateGRU(nn.Module):
    def __init__(self, pretrained_embed=None, trans_r_1=None, trans_r_2=None, embed_r=None, trans_wildcard=None,
        ...
        config: config
        """
        super(BRNNIntegrateEmptyStateSaperateGRU, self).__init__()
        ...
        _, rank = embed_r.shape  # R
        n_state, _ = trans_r_1.shape  # S
        ...
        self.embed_r = nn.Embedding.from_pretrained(torch.from_numpy(embed_r).float(),
                                                    freeze=(not bool(config.train_V_embed)))  # V x R

        self.trans_r_1 = nn.Parameter(
            torch.cat((torch.from_numpy(trans_r_1).float(), torch.randn((self.AS, self.R)) * config.random_noise),
                      dim=0),
            requires_grad=bool(config.train_fsa)
        )  # S x R
        self.trans_r_2 = nn.Parameter(
            torch.cat((torch.from_numpy(trans_r_2).float(), torch.randn((self.AS, self.R)) * config.random_noise),
                      dim=0),
            requires_grad=bool(config.train_fsa)
        )  # S x R
        embed_init_weight = self.embedding.weight.data  # V x D
        # print(embed_init_weight.shape)
    ...
    def cal_hidden_bar(self, hidden, L):
        ...
        if self.farnn == 1:
            self.zt = self.gate_activation(
                torch.matmul(hidden, self.Wss1) + torch.matmul(L, self.Wrs1) + self.bs1)  # B x S
    ...
    def forward(self, input, lengths):
        ...
        all_hidden = torch.zeros((B, L, self.S + self.AS)).cuda() if self.is_cuda else torch.zeros(
            (B, L, self.S + self.AS))
...
class IntentIntegrateSaperateBidirection_B(nn.Module):
    def __init__(self, pretrained_embed=None, forward_params=None, backward_params=None, config=None, h1_forward=None,
                 h1_backward=None):
        '''
        ...
        self.fsa_rnn_forward = BRNNIntegrateEmptyStateSaperateGRU(
        ...
        self.fsa_rnn_backward = BRNNIntegrateEmptyStateSaperateGRU(
        ...
        if not config.random:
            ...
            forward_mat = torch.cat(
                [forward_mat, torch.randn(config.additional_state, forward_mat.size()[1]) * config.random_noise])
            backward_mat = torch.from_numpy(backward_params['mat']).float()
            backward_mat = torch.cat(
                [backward_mat, torch.randn(config.additional_state, backward_mat.size()[1]) * config.random_noise])

            self.mat_forward = nn.Parameter(forward_mat, requires_grad=bool(config.train_linear))
            self.bias_forward = nn.Parameter(torch.from_numpy(forward_params['bias']).float(),
                                             requires_grad=bool(config.train_linear))
            self.mat_backward = nn.Parameter(backward_mat, requires_grad=bool(config.train_linear))
            self.bias_backward = nn.Parameter(torch.from_numpy(backward_params['bias']).float(),
                                              requires_grad=bool(config.train_linear))
        else:
            self.mat_forward = nn.Parameter(torch.randn(
                (forward_params['mat'].shape[0] + config.additional_state, forward_params['mat'].shape[1])).float(),
                                            requires_grad=bool(config.train_linear))
            self.bias_forward = nn.Parameter(torch.randn(forward_params['bias'].shape).float(),
                                             requires_grad=bool(config.train_linear))
            self.mat_backward = nn.Parameter(torch.randn(
                (backward_params['mat'].shape[0] + config.additional_state, backward_params['mat'].shape[1])).float(),
                                             requires_grad=bool(config.train_linear))
            self.bias_backward = nn.Parameter(torch.randn(backward_params['bias'].shape).float(),
                                              requires_grad=bool(config.train_linear))
...
class IntentIntegrateSaperate_B(nn.Module):
    def __init__(self, pretrained_embed=None, trans_r_1=None, trans_r_2=None, embed_r=None, trans_wildcard=None,
        ...
        self.fsa_rnn = BRNNIntegrateEmptyStateSaperateGRU(pretrained_embed=pretrained_embed,
                                                          trans_r_1=trans_r_1,
                                                          trans_r_2=trans_r_2,
                                                          embed_r=embed_r,
        ...
        if not config.random:
            self.mat = nn.Parameter(torch.cat([torch.from_numpy(mat).float(), torch.randn(config.additional_state,
                                                                                          mat.shape[
                                                                                              1]) * config.random_noise]),
                                    requires_grad=bool(config.train_linear))
            self.bias = nn.Parameter(torch.from_numpy(bias).float(), requires_grad=bool(config.train_linear))
        else:
            self.mat = nn.Parameter(torch.randn(mat.shape[0] + config.additional_state, mat.shape[1]).float(),
                                    requires_grad=bool(config.train_linear))
